...
const SEED_EXAMS = [
  { id: 1, subject: "Database Systems", date: "2026-07-08", venue: "Hall A" },
  { id: 2, subject: "Machine Learning", date: "2026-07-12", venue: "Hall B" },
  { id: 3, subject: "Software Engineering", date: "2026-07-15", venue: "" },
];
...
export default function RapidPrep() {
  ...
  const importantQuestions = VAULT_RESOURCES.filter((r) => r.isImportantQuestion);
  const previousPapers = VAULT_RESOURCES.filter((r) => r.isPYQ);
  ...
  const handleAddExam = () => {
    ...
    if (Object.keys(e).length) { setExamErrors(e); return; }
    setExams((prev) => [
      ...prev,
      {
        id: nextExamId++,
          ...
          <div className="rp-card-grid">
            {importantQuestions.map((r) => (
              <div className="rp-card" key={r.id}>
                ...
                <button className="rp-btn-primary">View →</button>
              </div>
            ))}
          </div>
        )}
