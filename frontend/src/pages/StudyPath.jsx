...
export default function StudyPath() {
  ...
  const [form, setForm] = useState({
    name: "", subject: "", days: [], activity: "", estTime: "", notes: "",
  });
    ...
    return DAYS.map((abbr) => {
      const entries = plans
        ...
        .map((p) => ({
          ...
          activity: p.activity,
          estTime: p.estTime,
          color: SUBJECT_COLORS[p.subject] || PALETTE[p.id % PALETTE.length],
        }));
  ...
  const validate = () => {
    ...
    if (form.days.length === 0) e.days = "Select at least one study day.";
    if (!form.activity.trim()) e.activity = "Daily activity / topic is required.";
  ...
  const closeModal = () => {
    setShowModal(false);
    setForm({ name: "", subject: "", days: [], activity: "", estTime: "", notes: "" });
    setErrors({});
  };
  ...
  const togglePlanComplete = (id) => {
    setPlans((prev) =>
      prev.map((p) =>
        p.id === id
          ? { ...p, completed: !p.completed, progress: !p.completed ? 100 : p.progress }
          : p
      )
    );
  };
      ...
      <section className="sp-card">
        ...
        <div className="sp-week-grid">
          {weekSchedule.map((day) => (
            <div className={`sp-day-col ${day.abbr === TODAY_ABBR ? "sp-day-today" : ""}`} key={day.day}>
              <div className="sp-day-header">{day.abbr}</div>
              {day.entries.length === 0 ? (
                <p className="sp-no-plan">No study planned.</p>
              ) : (
                day.entries.map((s, i) => (
                  <div className="sp-session" key={i}>
                    <p className="sp-session-time">{s.estTime}</p>
                    <p className="sp-session-task">{s.activity}</p>
                    <span
        ...
        <div className="sp-ring-wrap">
          <svg className="sp-ring" viewBox="0 0 120 120">
            ...
            <circle
              ...
              strokeDasharray={`${ringCircumference * (overallProgress / 100)} ${ringCircumference}`}
        ...
        <div className="sp-modal-overlay" onClick={closeModal}>
          <div className="sp-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="sp-modal-body">
              ...
              <div className="sp-form-group">
                <label className="sp-form-label">Study Days <span className="sp-required">*</span></label>
                <div className="sp-days-row">
                  {DAYS.map((d) => (
                    <button
                      key={d}
                      type="button"
                      className={`sp-day-chip ${form.days.includes(d) ? "sp-day-active" : ""}`}
                      onClick={() => { toggleFormDay(d); setErrors((er) => ({ ...er, days: undefined })); }}
                    >
                      {d}
                    </button>
                  ))}
                </div>
                {errors.days && <span className="sp-error-msg">{errors.days}</span>}
              </div>

              <div className="sp-form-group">
                <label className="sp-form-label">Daily Activity / Topic <span className="sp-required">*</span></label>
                <input
                  className={`sp-form-input ${errors.activity ? "sp-input-error" : ""}`}
                  type="text"
                  placeholder="e.g. Revise Transaction Management"
                  value={form.activity}
                  onChange={(e) => { setForm((f) => ({ ...f, activity: e.target.value })); setErrors((er) => ({ ...er, activity: undefined })); }}
                />
                {errors.activity && <span className="sp-error-msg">{errors.activity}</span>}
              </div>
